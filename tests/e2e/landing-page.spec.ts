...
test("публикует карту сайта для ИИ-агентов", async ({ request }) => {
...
test("публикует изображение для социальных превью", async ({ page, request }) => {
  ...
  expect(imageBytes.byteLength).toBeGreaterThan(40_000);
...
test("подсвечивает пункт навигации для текущей секции", async ({ page }) => {
  ...
  await expect(page.locator('.desktop-nav a[href="#projects"]')).toHaveAttribute(
    "aria-current",
    "location",
  );
});
...
test("выравнивает форму и контактную карточку по высоте", async ({ page }) => {
  test.skip(
    (page.viewportSize()?.width ?? 0) <= 760,
    "На мобильных панели расположены вертикально",
  );
  await page.goto("/#contacts");

  const formBox = await page.locator(".contact-form").boundingBox();
  const contactCardBox = await page.locator(".contact-card").boundingBox();

  expect(formBox && contactCardBox).toBeTruthy();
  expect(Math.abs(formBox!.height - contactCardBox!.height)).toBeLessThanOrEqual(1);
});
